...
def get_asset_preview(
    ...
    PDF behavior:
      ...
      - overlay: If page is provided, returns overlay image generated by analysis at previews/overlay_{page-stem}_p{page:04d}.png;
                 otherwise returns the original PDF.
      - fixed: If page is provided, returns fixed raster page under pdf/fixed_pages/{page:04d}.png when available, else original page;
    ...
    Notes:
        ...
        - Overlays are only available after analysis completes.
        - Fixed previews/pages are available after fix runs.
    """
    if state.get_job(job_id) is None:
        raise HTTPException(status_code=404, detail="Job not found")
